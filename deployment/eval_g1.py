...
from deployment.make_robot import (
    setup_image_client,
    setup_robot_interface,
    process_images_and_observations,
)
from deployment.utils.utils import (
    cleanup_resources,
    predict_action,
    to_list,
    to_scalar,
...
from deployment.utils.rerun_visualizer import RerunLogger, visualization_data
...
def eval_policy(
    ...
    try:
        ...
        logger_mp.info("Keyboard interrupt received. Shutting down...")
    except Exception as e:
        logger_mp.info(f"An error occurred: {e}")
    finally:
        # Send robot arm to home position before cleanup
        if "arm_ctrl" in locals() and arm_ctrl is not None:
            logger_mp.info("Sending robot arm to home position...")
            try:
                arm_ctrl.ctrl_dual_arm_go_home()
            except Exception as e:
                logger_mp.info(f"Error sending arm to home position: {e}")
