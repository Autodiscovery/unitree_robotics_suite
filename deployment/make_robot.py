...
import threading
import torch
from teleop.teleimager.image_client import ImageClient
...
from deployment.utils.episode_writer import EpisodeWriter

from teleop.robot_control.robot_hand_inspire import Inspire_Controller
from teleop.robot_control.robot_hand_inspire_fake import Inspire_Controller_Fake
from teleop.robot_control.robot_hand_brainco import Brainco_Controller
from teleop.robot_control.robot_hand_dummy import Dummy_Hand_Controller
...
ARM_CONFIG = {
    ...
    "H1_2": {"controller": H1_2_ArmController, "ik_solver": H1_2_ArmIK, "dof": 14},
    "H1": {"controller": H1_ArmController, "ik_solver": H1_ArmIK, "dof": 8},
}
...
EE_CONFIG: dict[str, dict[str, Any]] = {
    ...
    "inspire_fake": {
        "controller": Inspire_Controller_Fake,
        "dof": 6,
        "shared_mem_type": "Array",
        "shared_mem_size": 6,
        # "out_len": 12,
...
def setup_image_client(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if WRIST and getattr(args, "sim", False):
        ...
        img_client = ImageClient(
            ...
            wrist_img_shape=wrist_img_shape,
            wrist_img_shm_name=wrist_img_shm.name,
            host=getattr(args, 'image_server_ip', '192.168.123.164')
        )
    else:
        img_client = ImageClient(
            tv_img_shape=tv_img_shape, 
...
def setup_robot_interface(args: argparse.Namespace) -> dict[str, Any]:
    ...
    ee_key = getattr(args, "ee", "").lower()
    if ee_key and ee_key != "none":
    ...
    if is_sim:
        ...
        from deployment.utils.sim_state_topic import (
...
def process_images_and_observations(
    ...
    observation = {
        "observation.images.cam_high": torch.from_numpy(left_top_cam),
        # "observation.images.cam_leftobservation.images.cam_right_high_high": torch.from_numpy(left_top_cam),
